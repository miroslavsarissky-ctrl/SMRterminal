...
The terminal draws from the databases behind the federal registers, the regulators, the SEC, the legislature, the national labs, energy statistics and the nuclear trade press, then presents the results as a filterable feed alongside a deadline radar, a legislation tracker and a state-level energy map. It is built for United States nuclear market-intelligence work: watching the advanced-reactor and fuel-cycle landscape, the regulatory and funding pipeline, and the companies and agencies that matter to that space.

The design is deliberately simple to host. There is no server to run: a scheduled job queries the source databases directly, a Python collector normalises what they return and writes the results as data files into the repository, and a single self-contained HTML file reads those files in the browser. Everything is committed to the repo and served by GitHub Pages.
...
GitHub Actions (cron, every 30 minutes, or run on demand)
  └─ runs  terminal/collectors/collect.py
       ├─ queries each source database, normalises items, applies per-source throttles
...
Because each run commits fresh snapshots from the source databases, the page has no runtime dependencies and loads instantly. The trade-off is that the feed is only as fresh as the last successful collector run.
...
Each source below is a public database or feed that the collector queries on the listed cadence.
...
The EDGAR database is queried two ways: per-company submissions for the watchlist, plus a full-text search on a configurable issuer keyword that surfaces mentions of tracked entities inside other parties' filings, which is how a de-SPAC trail can be followed across sponsor and counterparty documents. Sources with no key requirement work out of the box; the rest stay dark until their key is supplied.
...
Energy data comes from the EIA database via API v2, specifically `electricity/retail-sales` (price by state and sector) and `electricity/operating-generator-capacity` (nuclear capacity and unit count by state). The retail series lags roughly two months, so the map shows the latest published month rather than the live week.
...
**A source stays empty.** Almost always the secret exists but is not wired into the Collect step's `env:` block (see Configuration above). The collector skips keyless sources silently, so there is no error to find.

**The workflow fails before the collector runs.** A run that dies in the "Prepare all required actions" phase with `Failed to resolve action download info. Error: Service Unavailable` is a GitHub-side outage in the Actions service, not anything in this repository: the runner is failing to download the workflow's own actions before any project code executes. Re-run the job from the Actions tab for an immediate retry; otherwise the next 30-minute scheduled run clears it on its own, at the cost of one missed cycle. GitHub's status page (githubstatus.com) usually confirms the incident, though brief blips are not always logged there.

**The page shows stale data.** Check the last green run under **Actions → refresh-intel**. If the runs are green, the browser is showing a cached copy of the page; hard-refresh (Cmd/Ctrl+Shift+R) to clear the Pages cache.
...
Built and maintained by the Communications & Market Intelligence team.
